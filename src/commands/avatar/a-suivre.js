...
module.exports = class ASuivreCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'a-suivre',
            type: client.types.AVATAR,
			description: 'Dessine votre avatar avec la flèche "À suivre ...".',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'a-suivre.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'a-suivre.png' }] });
		} catch (err) {
