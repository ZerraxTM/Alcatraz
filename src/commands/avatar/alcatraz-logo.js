...
module.exports = class RainbowCommand extends Command {
	constructor(client) {
		super(client, {
            ...
            description: 'Dessine un le logo d\'Alcatraz sur un avatar d\'un utilisateur.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'alcatraz-logo.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'alcatraz-logo.png' }] });
		} catch (err) {
