...
module.exports = class ASMonacoCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'asmonaco',
            type: client.types.AVATAR,
            description: 'Avoir un filtre avec le logo de l\'AS Monaco.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'asmonaco.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'asmonaco.png' }] });
		} catch (err) {
