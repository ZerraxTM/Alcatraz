...
module.exports = class AsseCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'asse',
            type: client.types.AVATAR,
            description: 'Avoir un filtre avec le logo de Saint étienne.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'asse.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'asse.png' }] });
		} catch (err) {
