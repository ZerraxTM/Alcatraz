...
module.exports = class LoscCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'losc',
            type: client.types.AVATAR,
            description: 'Avoir un filtre avec le logo du Losc Lille.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'losc.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'losc.png' }] });
		} catch (err) {
