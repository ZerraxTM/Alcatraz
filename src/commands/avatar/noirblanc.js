...
module.exports = class NoirBlancCommand extends Command {
	constructor(client) {
		super(client, {
            name: 'noirblanc',
            type: client.types.AVATAR,
			description: 'Avoir un avatar en noir et blanc.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			...
			return msg.channel.send({ files: [{ attachment, name: 'noirblanc.png' }] });
		} catch (err) {
