...
module.exports = class OlCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'ol',
            type: client.types.AVATAR,
            description: 'Avoir un filtre avec le logo de l\'Olympique Lyonnais.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'ol.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'ol.png' }] });
		} catch (err) {
