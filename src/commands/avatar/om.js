...
module.exports = class OmCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'om',
            type: client.types.AVATAR,
            description: 'Avoir un filtre avec le logo de l\'Olympique de Marseille.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'om.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'om.png' }] });
		} catch (err) {
