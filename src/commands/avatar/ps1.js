...
module.exports = class PS1Command extends Command {
    constructor(client) {
        super(client, {
            name: 'ps1',
            type: client.types.AVATAR,
            description: 'Avoir une jaquette PS1 avec son avatar.',
            throttling: {
                usages: 1,
                duration: 10
            },
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'ps1.png'));
            ...
            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'ps1.png' }] });
        } catch (err) {
