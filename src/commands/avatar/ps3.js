...
module.exports = class PS3Command extends Command {
    constructor(client) {
        super(client, {
            name: 'ps3',
            type: client.types.AVATAR,
            description: 'Avoir une jaquette PS3 avec son avatar.',
            throttling: {
                usages: 1,
                duration: 10
            },
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'ps3.png'));
            ...
            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'ps3.png' }] });
        } catch (err) {
