...
module.exports = class PS5Command extends Command {
    constructor(client) {
        super(client, {
            name: 'ps5',
            type: client.types.AVATAR,
            description: 'Avoir une jaquette PS5 avec son avatar.',
            throttling: {
                usages: 1,
                duration: 10
            },
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'ps5.png'));
            ...
            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'ps5.png' }] });
        } catch (err) {
