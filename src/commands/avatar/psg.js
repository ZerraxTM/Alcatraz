...
module.exports = class AlcatrazFiltreCommand extends Command {
	constructor(client) {
		super(client, {
            ...
            description: 'Avoir un filtre avec le logo du Paris Saint-Germain.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { image, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'psg.png'));
			...
			return msg.channel.send({ files: [{ attachment, name: 'psg.png' }] });
		} catch (err) {
