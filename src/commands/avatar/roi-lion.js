...
module.exports = class RoiLionCommand extends Command {
	constructor(client) {
		super(client, {
            name: 'roi-lion',
            type: client.types.AVATAR,
            description: 'Dessine l\'avatar d\'un utilisateur sur Simba à partir de la réflexion du Roi Lion.',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { user, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'roi-lion.png'));
			...
			ctx.rotate(24 * (Math.PI / 180));
			return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'roi-lion.png' }] });
		} catch (err) {
