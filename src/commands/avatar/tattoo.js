...
module.exports = class TattooCommand extends Command {
	constructor(client) {
		super(client, {
            name: 'tattoo',
            type: client.types.AVATAR,
            description: 'Dessine l\'avatar d\'un en mode tatouage',
			throttling: {
				usages: 1,
				duration: 10
			},
	...
	async run(msg, { user, args}) {
      ...
      msg.member;
		...
		try {
			const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'tattoo.png'));
			...
			ctx.rotate(10 * (Math.PI / 180));
			return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'tattoo.png' }] });
		} catch (err) {
