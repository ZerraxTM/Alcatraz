...
module.exports = class TF1Command extends Command {
    constructor(client) {
        super(client, {
            name: 'tf1',
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'tf1.png'));
            ...
            ctx.fillText(member.user.tag, 230, 166, 305);
            ctx.fillStyle = 'white';
            if (cause) ctx.fillText(cause, 438, 910, 500);

            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'tf1.png' }] });
        } catch (err) {
