...
module.exports = class TvCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            description: 'Avoir votre avatar sur une télé dans le salon.',
            throttling: {
                usages: 1,
                duration: 10
            },
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'tv.png'));
            ...
            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'tv.png' }] });
        } catch (err) {
