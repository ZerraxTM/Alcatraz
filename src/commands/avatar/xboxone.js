...
module.exports = class XboxOneCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'xboxone',
            type: client.types.AVATAR,
            description: 'Avoir une jaquette XboxOne avec son avatar.',
            throttling: {
                usages: 1,
                duration: 10
            },
    ...
    async run(msg, { user, cause, args }) {
      ...
      msg.member;
        ...
        try {
            const base = await loadImage(path.join(__dirname, '..', '..', 'assets', 'images', 'xboxone.png'));
            ...
            return msg.channel.send({ files: [{ attachment: canvas.toBuffer(), name: 'xboxone.png' }] });
        } catch (err) {
